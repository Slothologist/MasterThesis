...
\begin{figure}[]
	...
	\caption{Test scenario for existing pipeline.
		The PocketSphinxAdapter acquires audio data from ALSA and provides speech recognitions.
...
\begin{figure}[]
	...
	\caption{Test scenario for proposed pipelines baseline.
		...
		The audio is then processed by a VAD and a speech recognizer.
...
\begin{figure}[]
	...
	\caption{Test scenario for an elongated configuration of the proposed pipeline.
		In comparison to the baseline of the proposed pipeline, it encompasses an additional dummy node which is squeezed in between the wav player and the VAD.
