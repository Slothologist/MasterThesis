...
In chapter \ref{main:lib}, we discussed the fact that each component will register itself giving a designation.
...
\begin{figure}[]
	...
	\begin{tabular}{| l | r |}
		...
		Other 				& None	 	\\ \hline
	\end{tabular}
	\caption{Possible designations of components along with result messages expected from them.}
...
To take care of all results the Orchestrator has already received, we chose to create a database, in which all incoming results are saved.
...
Additionally, we again must bear in mind that components may unexpectedly shut down.
...
This is somewhat contrary to our previously discussed goal of ensuring the synchronization of all results, but is crucial to ensure all results to be actually relayed to further programs and arrive there with a minimal latency.
...
The method we developed to find the optimal timeout takes into account all of these concerns.
Making use of the database we included to save results, we do not only save received results, but also their time to compute.
This can very easily be calculated, as we can just take the difference between the time the result is received by the Orchestrator and the end time of the audio timestamp.
...
Because we generate this time for each result, we can then calculate the average for each type of result.
Thus we can generate a timeout for each type of result which is equivalent to a timeout for each component.
To avoid ignoring results which take shortly longer to arrive at the Orchestrator than the average, we multiply the average value with a constant factor to gain the final timeout.
...
The first anchor result the Orchestrator receives however will not be able to generate timeouts this way, we need an alternative for it.
...
However, at this point another feature of our database approach from before comes in.
As our results are stored permanently, we are able to simply load an already existing database.
...
Lastly we will take a look at the latency introduced by synchronizing results.
...
This latency $l_f$ is mostly determined by the exact factors we discussed previously: The maximum of the average time to compute of all components and the multiplicand $V_A$. 
